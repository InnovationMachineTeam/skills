...
Использовать категории только для устойчивых доменов и ownership-политик. Для текущего портфеля уместна категория `metaskills`. Будущие категории добавлять по мере появления содержимого.
