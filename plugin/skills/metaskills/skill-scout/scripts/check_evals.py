#!/usr/bin/env python3
"""Validate skill-scout routing and behavior eval datasets."""
...
ROUTES = {"session-insights", "corpus-mining", "existing-coverage", "feasibility-context", "portfolio-prioritization", "handoff"}
...
def load(path: Path, findings: list[Finding]) -> dict[str, object] | None:
    try:
        ...
        return None
    if not isinstance(value, dict) or value.get("skill") != "skill-scout":
        findings.append(Finding("shape", "Eval root must identify skill-scout.", str(path)))
        return None
    return value
...
def validate(eval_dir: Path) -> list[Finding]:
    ...
    if routing is not None:
        ...
        for required in ("clarify", "scout", "route-specialist", "do-not-trigger"):
