#!/usr/bin/env python3
"""Validate skill-doctor routing and behavior eval datasets."""
...
def load(path: Path, findings: list[Finding]) -> dict[str, object] | None:
    ...
    if not isinstance(value, dict):
        ...
        return None
    if value.get("skill") != "skill-doctor":
        findings.append(Finding("skill", "Eval must identify skill-doctor.", str(path)))
    return value


def get_cases(value: dict[str, object], path: Path, findings: list[Finding]) -> list[dict[str, object]]:
    raw = value.get("cases")
...
def validate(eval_dir: Path) -> list[Finding]:
    ...
    if routing is not None:
        routing_cases = get_cases(routing, routing_path, findings)
        routes = {case.get("expected_primary_domain") for case in routing_cases}
        missing = DOMAINS - routes
        if missing:
            findings.append(Finding("domain-coverage", "Missing domains: " + ", ".join(sorted(missing)), str(routing_path)))
        ...
        for required in ("clarify", "diagnose", "diagnose-and-repair", "do-not-trigger"):
    ...
    if behavior is not None:
        for case in get_cases(behavior, behavior_path, findings):
