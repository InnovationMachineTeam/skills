#!/usr/bin/env python3
"""Validate skill-harvester routing and behavior eval datasets."""
...
ROUTES = {
    "source-inventory", "patterns-workflows", "knowledge-references", "prompts-templates",
    "scripts-tools", "evals-failures", "synthesis-dedup", "integration-dispatch",
    "context-build", "pairwise-skill-comparison", "external-skill-intake",
}
...
def load(path: Path, findings: list[Finding]) -> dict[str, object] | None:
    ...
    if not isinstance(value, dict):
        ...
        return None
    if value.get("skill") != "skill-harvester":
        findings.append(Finding("skill", "Eval must identify skill-harvester.", str(path)))
    return value
...
def validate(eval_dir: Path) -> list[Finding]:
    ...
    if routing is not None:
        ...
        missing = ROUTES - {case.get("expected_primary_route") for case in routing_cases}
        if missing:
            findings.append(Finding("route-coverage", "Missing routes: " + ", ".join(sorted(missing)), str(routing_path)))
        if not {True, False}.issubset({case.get("expected_trigger") for case in routing_cases}):
        ...
        for required in ("clarify", "inventory", "harvest", "route-specialist", "do-not-trigger"):
