...
`skill-manager` управляет жизненным циклом портфеля агентных навыков: инвентаризирует явно заданные корни, выявляет конфликты, планирует установку и обновление, управляет доступностью, проверяет цепочку поставки, помогает с governance и безопасным выводом навыков из эксплуатации.
...
Скрипт работает только с явно переданными корнями, не исполняет содержимое навыков, вычисляет детерминированные хеши и отмечает предполагаемые конфликты по порядку корней. Сканирование `/` и домашнего каталога отклоняется.
