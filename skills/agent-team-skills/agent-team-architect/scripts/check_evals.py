#!/usr/bin/env python3
"""Validate agent-team-architect routing and behavior fixtures."""
...
ROUTES = {"worth-assessment", "design-team", "redesign-team", "topology", "worktree-policy"}
...
def main() -> int:
    ...
    for filename in ("routing.json", "behavior.json"):
        ...
        if filename == "routing.json":
            ...
            for case in cases:
                if not isinstance(case, dict) or not isinstance(case.get("expected_properties"), list) or not isinstance(case.get("forbidden_properties"), list):
                    failures.append(f"{path}: behavior properties required")
    for failure in failures:
        print(f"FAIL {failure}")
    if failures:
        return 1
    print("PASS agent-team-architect eval fixtures")
    return 0
