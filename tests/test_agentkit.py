#!/usr/bin/env python3
"""Stable packaging, donor-lock, E2E, rollback, and approval gates for agentkit."""
...
AGENTKIT = ROOT / "skills" / "agent-skills" / "agentkit"
DONORS = {
...
class AgentkitTests(unittest.TestCase):
    def test_stable_skill_is_discoverable_and_selectively_packaged(self) -> None:
        self.assertTrue((AGENTKIT / "SKILL.md").is_file())
        self.assertFalse((ROOT / "candidates/agentkit").exists())
        catalog = {item["name"] for item in load(ROOT / "catalog/entries.json")["entries"]}
        self.assertIn("agentkit", catalog)
        self.assertTrue((ROOT / "plugins/agentkit/skills/agentkit/SKILL.md").exists())
        for path in (
            ROOT / ".claude-plugin/marketplace.json",
            ROOT / ".agents/plugins/marketplace.json",
            ROOT / ".cursor-plugin/marketplace.json",
        ):
            self.assertIn("agentkit", {item["name"] for item in load(path)["plugins"]})

    def test_lock_and_vendor_are_exact_and_read_only(self) -> None:
        manifest = load(AGENTKIT / "donors.json")
        self.assertEqual(DONORS, {item["name"] for item in manifest["donors"]})
        self.assertEqual(10, len(manifest["donors"]))
        self.assertFalse(any((AGENTKIT / "vendor").rglob("SKILL.md")))
        script = AGENTKIT / "scripts/check_donors.py"
        result = subprocess.run([
            str(script), "--manifest", str(AGENTKIT / "donors.json"),
            "--vendor-root", str(AGENTKIT / "vendor"),
            "--source-root", str(ROOT / "skills/agent-skills"),
        ], check=False, capture_output=True, text=True)
        self.assertEqual(0, result.returncode, result.stderr)
        with tempfile.TemporaryDirectory() as directory:
            drift = load(AGENTKIT / "donors.json")
            ...
            result = subprocess.run([
                str(script), "--manifest", str(path), "--vendor-root", str(AGENTKIT / "vendor")
            ], check=False)
            self.assertEqual(2, result.returncode)

    def test_routing_is_explicit_and_does_not_claim_direct_agent_tasks(self) -> None:
        routing = load(AGENTKIT / "evals/routing.json")["cases"]
        ...
        body = (AGENTKIT / "SKILL.md").read_text(encoding="utf-8")
        self.assertIn("explicit `$agentkit` invocation", body)
        self.assertIn("Ask for explicit approval", (AGENTKIT / "references/e2e-contract.md").read_text(encoding="utf-8"))

    def test_e2e_scaffold_and_false_completion_gate(self) -> None:
        scaffold = AGENTKIT / "scripts/scaffold_e2e_run.py"
        validate = AGENTKIT / "scripts/validate_e2e_run.py"
        with tempfile.TemporaryDirectory() as directory:
            ...
            runner = AGENTKIT / "scripts/run_router_fixture.py"
            self.assertEqual(0, subprocess.run([
                str(runner), "--run", str(run), "--manifest", str(AGENTKIT / "donors.json")
    ...
    def test_real_workflow_and_release_contracts_fail_closed(self) -> None:
        scaffold = AGENTKIT / "scripts/scaffold_e2e_run.py"
        finalize = AGENTKIT / "scripts/record_real_workflow.py"
        verify = AGENTKIT / "scripts/verify_release_contracts.py"
        rollback = AGENTKIT / "scripts/build_rollback_plan.py"
        self.assertEqual(0, subprocess.run([
            str(verify), "--pack-root", str(AGENTKIT), "--repository-root", str(ROOT)
        ], check=False).returncode)
        holdout = load(ROOT / "docs/agents/evals/agentkit-pack-holdout-v1.json")
        self.assertEqual(7, len(holdout["cases"]))
        self.assertTrue(holdout["exposure"]["outside_distributed_pack"])
        self.assertFalse((AGENTKIT / "docs/agents/evals/agentkit-pack-holdout-v1.json").exists())
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            run = root / "run"
            self.assertEqual(0, subprocess.run([
            ...
            self.assertNotEqual(0, subprocess.run([
                str(finalize), "--run", str(run), "--manifest", str(AGENTKIT / "donors.json"), "--spec", str(spec)
            ], check=False).returncode)
            plan = root / "rollback.json"
            self.assertEqual(0, subprocess.run([
                str(rollback), "--manifest", str(AGENTKIT / "donors.json"), "--output", str(plan),
    ...
    def test_donor_finding_requires_matching_user_approval(self) -> None:
        classify = AGENTKIT / "scripts/classify_e2e_findings.py"
        render = AGENTKIT / "scripts/render_improvement_prompt.py"
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            self.assertEqual(0, subprocess.run([
                str(classify), "--findings", str(findings), "--manifest", str(AGENTKIT / "donors.json"),
